...
for ((i=0;i<${#input_files[@]};++i)); do
  ...
  python -m sacrerouge correlate \
    ...
    --metrics ${judgment_metrics[i]} MoverScore \
  ...
  python -m sacrerouge correlate \
    --metrics-jsonl-files ${metrics_files[i]} ${DIR}/output/${output_dirs[i]}/scores.jsonl \
    --metrics ${judgment_metrics[i]} MoverScore_jk \
    --summarizer-type all \
...
for summarizer_type in 'all' 'peer'; do
  for level in 'summary_level' 'system_level'; do
    python -m sacrerouge.scripts.create_correlations_table \
      ...
      ${level} \
      --input ${DIR}/output/duc2005/correlations/${summarizer_type}.json MoverScore DUC2005 \
      --input ${DIR}/output/duc2006/correlations/${summarizer_type}.json MoverScore DUC2006 \
      --input ${DIR}/output/duc2007/correlations/${summarizer_type}.json MoverScore DUC2007 \
      --input ${DIR}/output/tac2008/correlations/${summarizer_type}.json MoverScore TAC2008 \
      --input ${DIR}/output/tac2009/correlations/${summarizer_type}.json MoverScore TAC2009 \
      --input ${DIR}/output/tac2010/correlations/${summarizer_type}.json MoverScore TAC2010 \
      --input ${DIR}/output/tac2011/correlations/${summarizer_type}.json MoverScore TAC2011
  done
done