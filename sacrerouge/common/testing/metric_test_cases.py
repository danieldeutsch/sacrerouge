...
class MetricTestCase(unittest.TestCase):
    ...
    def _assert_order_invariant(self, metric: Metric, *args, abs: float = None):
        ...
        for metrics_list1, metrics_list2 in zip(metrics_lists1, metrics_lists2):
            for metrics1, metrics2 in zip(metrics_list1, metrics_list2):
                assert metrics1.approx_equal(metrics2, abs=abs)
...
class DocumentBasedMetricTestCase(MetricTestCase):
    ...
    def assert_order_invariant(self, metric: Metric, abs: float =None):
        self._assert_order_invariant(metric, self.documents_list, abs=abs)
...
class ReferenceBasedMetricTestCase(MetricTestCase):
    ...
    def assert_order_invariant(self, metric: Metric, abs: float =None):
        self._assert_order_invariant(metric, self.references_list, abs=abs)
...
class ReferencelessMetricTestCase(MetricTestCase):
    ...
    def assert_order_invariant(self, metric: Metric, abs: float =None):
        self._assert_order_invariant(metric, abs=abs)